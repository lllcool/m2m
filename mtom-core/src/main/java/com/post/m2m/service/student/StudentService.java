...
public class StudentService {
     ...
     */
    @Transactional(rollbackFor = RuntimeException.class)
    public int batchSave(List<StudentAddDTO> list) {
     ...
     * @return
     */
    public PageVO<StudentListVO> list(StudentQO studentQO) {
        PageVO<StudentListVO> page = studentDAO.findByPage(studentQO);
        return page;
    }
    ...
    public List<OptionVO<String, String>> findOptions(OptionQO<String, String> qo) {
        List<OptionVO<String, String>> options = studentDAO.findOptions(qo);
        return options;
    }
     ...
     * @return
     */
    public StudentPO getStudent(String id, boolean force) {
    ...
    private void checkDeleteByStuRefTeach(String id) {
        int count = stuRefTeachDAO.getCountBySId(id);
        if (count > 0) {
